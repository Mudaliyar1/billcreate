...
exports.postCreateBill = async (req, res) => {
  try {
    ...
    if (paidAmount) {
      ...
      billData.remainingAmount = totalAmount - parsedAmount;
      ...
      billData.paidAmount = totalAmount;
      ...
      for (const item of bill.items) {
        ...
        if (product) {
          ...
          if (originalPaidAmount < bill.totalAmount && newPaidAmount >= bill.totalAmount) {
            ...
            const originalPaidAmount = itemTotal * originalPaidRatio;
            const originalCreditAmount = itemTotal * (1 - originalPaidRatio);

            const newPaidAmount = itemTotal * newPaidRatio;
            const newCreditAmount = itemTotal * (1 - newPaidRatio);

            // Update the amounts
            product.totalPaidAmount = product.totalPaidAmount - originalPaidAmount + newPaidAmount;
            product.totalCreditAmount = product.totalCreditAmount - originalCreditAmount + newCreditAmount;
